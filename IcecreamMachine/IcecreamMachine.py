...
class Usable:
    ...
    def __init__(self, name, quantity = 10, cost=99):
        ...
        self.cost = cost
    # my ucid is ar2576 and the date try and except block was included to handle the exceptions is 10/23/2022
    def use(self):
        self.quantity -= 1
        try:
           if (self.quantity < 0):
            ...
            print("The option you have selected is currently out of stock, Could you please select another option.")
        else:
...
class STAGE(Enum):
    ...
    Toppings = 3
    Pay = 4
# my ucid is ar2576 and I initialized flavr and top as global variables and then created an array of empty list of both the varibles in 10/22/2022
class IceCreamMachine:
    ...
    # My ucid is ar2576 and the date code was modified is 10/22/2022
    def pick_container(self, choice):
        for c in self.containers:
            if c.name.lower() == choice:
                ...
                return
        print(InvalidChoiceException.display)
        IceCreamMachine.start(self)

    # My ucid is ar2576 and the date code was modified is 10/22/2022
    def pick_flavor(self, choice):
        if self.remaining_uses <= 0:
            print(NeedsCleaningException.display)
            IceCreamMachine.clean_machine(self)
            flavr.pop()
            IceCreamMachine.run(self)
        if self.remaining_scoops <= 0:
            print(ExceededRemainingChoicesException.display)
            flavr.pop()
    ...
    # My ucid is ar2576 and the date code was implelemented is 10/22/2022
    def handle_pay(self, expected, total):
        if str(total) == str(expected):
            ...
            print(InvalidPaymentException.display)
            IceCreamMachine.start(self)

    # My ucid is ar2576 and the date calculate_cost function was implemented is 10/22/2022
    def calculate_cost(self, cont, flav, topin):
        # TODO add the calculation expression/logic for the inprogress_icecream
        global cost
        cost = 0
        for con in self.containers:
            if con.name.lower() == cont:
                cost = con.cost
                break
        print('After container cost is {}'.format(cost))
        for count in range(len(flav)):
            if flav[count] == 'next':
                break
            elif self.flavors[0].name.lower() == flav[count]:
                cost += self.flavors[0].cost
            elif self.flavors[1].name.lower() == flav[count]:
                cost += self.flavors[1].cost
            elif self.flavors[2].name.lower() == flav[count]:
                cost += self.flavors[2].cost
            else: 
                pass
    # My ucid is ar2576 and the date calculate_cost function was implemented is 10/22/2022
        print('After flavor cost is {}'.format(cost))
        for counter in range(len(topin)):
            if topin[counter] == 'done':
                break
            elif self.toppings[0].name.lower() == topin[counter]:
                cost += self.toppings[0].cost
            elif self.toppings[1].name.lower() == topin[counter]:
                cost += self.toppings[1].cost
            elif self.toppings[2].name.lower() == topin[counter]:
                cost += self.toppings[2].cost
            elif self.toppings[3].name.lower() == topin[counter]:
                cost += self.toppings[3].cost
            elif self.toppings[4].name.lower() == topin[counter]:
                cost += self.toppings[4].cost
            else:
                pass
        print('After topping cost is {}'.format(cost))
        return cost
    # My ucid is ar2576 and the date code was modified is 10/22/2022
    def run(self):
        global container
        if self.currently_selecting == STAGE.Container:
            ...
            expected = self.calculate_cost(container, flavr, top)
