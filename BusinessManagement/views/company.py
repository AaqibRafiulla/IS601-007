...
def search():
    ...
    # don't do SELECT *
    # Added SQL query to retrieve company data - Aaqib Rafiulla,ar2576, Nov30, 2022
    query = """
    ...
    # Added code to request the comapny records from request args - Aaqib Rafiulla,ar2576, Nov30, 2022
    requestArgs = request.args.to_dict()
    # TODO search-3 append a LIKE filter for name if provided
    # Added code to append using like filter for name - Aaqib Rafiulla,ar2576, Nov30, 2022
    ...
    # Added code to sort if column and order are provided and also within the allowed columns and order options - Aaqib Rafiulla, ar2576, Nov 30, 2022
    ...
    # if limit isn't a number or if it's out of bounds - Aaqib Rafiulla, ar2576, Nov 30, 2022
    ...
    try:
        ...
        # Added code to make the message user friendly - Aaqib Rafiulla, ar2576, Nov 30, 2022
...
def add():
    if request.method == "POST":
        ...
        # Added code to get state and print flash error message - Aaqib Rafiulla, ar2576, Dec 1, 2022
...
def edit():
    ...
    if company_id: # TODO update this for TODO edit-1
        if request.method == "POST":
            ...
            # Added code to retrieve form data for name, address, city ... - Aaqib Rafiulla, ar2576, Dec 2,2022
            formData = request.form.to_dict()
            # TODO edit-3 name is required (flash proper error message)
            # Added code get the name and print a flash error message - Aaqib Rafiulla, ar2576, Dec 2,2022
            if not formData.get('name'):
                flash('Name is required!','danger')
                has_error=True
            # TODO edit-4 address is required (flash proper error message)
            # Added code to get the address and print a flash error message - Aaqib Rafiulla, ar2576, Dec 2,2022
            if not formData.get('address'):
                flash('Address is required!','danger')
                has_error=True
            # TODO edit-5 city is required (flash proper error message)
            # Added code to get the city and print a flash error message - Aaqib Rafiulla, ar2576, Dec 2,2022
            if not formData.get('city'):
                flash('City is required!','danger')
                has_error=True
            # TODO edit-6 state is required (flash proper error message)
            # Added code get the state and print a flash error message - Aaqib Rafiulla, ar2576, Dec 2,2022
            if not formData.get('state'):
                flash('State is required!','danger')
                has_error=True
            # TODO edit-7 country is required (flash proper error message)
            # Added code get the country and print a flash error message - Aaqib Rafiulla, ar2576, Dec 2,2022
            ...
            try:
                ...
                # Added code to make it user freindly - Aaqib Rafiulla, ar2576, Dec 2,2022
                print('An error occurred during updation of company',e)
                flash('An error occurred during updation of company', "danger")
        try:
            # TODO edit-11 fetch the updated data
            # Added query to fetch the updated data from company - Aaqib Rafiulla, ar2576, Dec 2,2022
    ...
    # Added code to pass the employee data to the render template - Aaqib Rafiulla, ar2576, Dec 3,2022
    return render_template("edit_company.html", row=row)

@company.route("/delete", methods=["GET"])
def delete():
    # TODO delete-1 delete company by id (unallocate any employees)
    # Added code to delete company by uid - Aaqib Rafiullla, ar2576, Dec 3, 2022
    company_id = request.args.get('id')
    has_error = False
    if company_id:
        try:
    ...
    # Added code to todo items for delete-2, delete-3, delete-4 - Aaqib Rafiulla, ar2576, Dec 3,2022
    if not has_error:
